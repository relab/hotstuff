...
// ProposePiped is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) ProposePiped(ctx context.Context, in *PipedProposal, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "hotstuffpb.Hotstuff.ProposePiped",
	}
...
type Hotstuff interface {
	...
	ProposePiped(ctx gorums.ServerCtx, request *PipedProposal)
}
...
func RegisterHotstuffServer(srv *gorums.Server, impl Hotstuff) {
	...
	srv.RegisterHandler("hotstuffpb.Hotstuff.ProposePiped", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*PipedProposal)
		defer ctx.Release()
		impl.ProposePiped(ctx, req)
	})
}
