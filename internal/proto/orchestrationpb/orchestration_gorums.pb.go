...
// with the argument returned by the provided function f, and returns the combined result.
// The per node function f receives a copy of the CreateClientRequest request argument and
// returns a CreateClientRequest manipulated to be passed to the given nodeID.
// The function f must be thread-safe.
func (c *Configuration) CreateClient(ctx context.Context, in *CreateClientRequest, f func(*CreateClientRequest, uint32) *CreateClientRequest) (resp *CreateClientResponse, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "orchestrationpb.Orchestrator.CreateClient",
	}
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
		r := make(map[uint32]*CreateClientResponse, len(replies))
		for k, v := range replies {
			r[k] = v.(*CreateClientResponse)
		}
		return c.qspec.CreateClientQF(req.(*CreateClientRequest), r)
	}
	cd.PerNodeArgFn = func(req protoreflect.ProtoMessage, nid uint32) protoreflect.ProtoMessage {
		return f(req.(*CreateClientRequest), nid)
	}
...
type internalCreateClientResponse struct {
	nid   uint32
	reply *CreateClientResponse
	err   error
}
