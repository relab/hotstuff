...
func RegisterHotstuffServer(srv *gorums.Server, impl Hotstuff) {
	srv.RegisterHandler("msg.Hotstuff.Propose", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*Proposal)
		defer ctx.Release()
		impl.Propose(ctx, req)
	})
	srv.RegisterHandler("msg.Hotstuff.Vote", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*PartialCert)
		defer ctx.Release()
		impl.Vote(ctx, req)
	})
	srv.RegisterHandler("msg.Hotstuff.Timeout", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*TimeoutMsg)
		defer ctx.Release()
		impl.Timeout(ctx, req)
	})
	srv.RegisterHandler("msg.Hotstuff.NewView", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*SyncInfo)
		defer ctx.Release()
		impl.NewView(ctx, req)
	})
	srv.RegisterHandler("msg.Hotstuff.Fetch", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
