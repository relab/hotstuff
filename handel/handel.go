...
func (h *Handel) Init() error {
	...
	h.sessions = make(map[msg.Hash]*session)
	...
	h.mods.EventLoop().RegisterHandler(contribution{}, func(event any) {
		...
		if s, ok := h.sessions[c.hash]; ok {
			...
			c.deferred = true
			h.mods.EventLoop().DelayUntil(msg.ProposeMsg{}, c)
		}
	})
...
func (h *Handel) Begin(s msg.PartialCert) {
...
type contribution struct {
	...
	sender     hotstuff.ID
	level      int
	signature  msg.QuorumSignature
	individual msg.QuorumSignature
	verified   bool
	deferred   bool
	score      int
}
