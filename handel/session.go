...
func (h *Handel) newSession(hash msg.Hash, in msg.QuorumSignature) *session {
	s := &session{
		h:    h,
		hash: hash,
...
type level struct {
	...
	incoming   msg.QuorumSignature
	outgoing   msg.QuorumSignature
	individual map[hotstuff.ID]msg.QuorumSignature
...
func (s *session) newLevel(i int) level {
	return level{
		...
		individual: make(map[hotstuff.ID]msg.QuorumSignature),
	}
}

func (s *session) canMergeContributions(a, b msg.QuorumSignature) bool {
	canMerge := true
...
func (s *session) score(contribution contribution) int {
	...
	finalParticipants := msg.NewIDSet()
...
func (s *session) improveSignature(contribution contribution) msg.QuorumSignature {
	level := &s.levels[contribution.level]
...
func (s *session) verifyContribution(c contribution, sig msg.QuorumSignature, verifyIndiv bool) {
