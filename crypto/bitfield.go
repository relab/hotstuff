...
func index(id consensus.ID) (byteIdx, bitIdx int) {
	i := int(id) - 1
	byteIdx = i / 8
	bitIdx = i % 8
	return
...
func id(byteIdx, bitIdx int) consensus.ID {
	return consensus.ID(1 + (byteIdx * 8) + bitIdx)
}

// Add adds an ID to the set.
func (bm *Bitfield) Add(id consensus.ID) {
...
func (bm *Bitfield) Contains(id consensus.ID) bool {
	byteIdx, bitIdx := index(id)
	if len(*bm) <= byteIdx {
		return false
	}
...
func (bm *Bitfield) ForEach(f func(consensus.ID)) {
