...
/* Exported types and methods for quorum call method Propose */
...
func (c *Configuration) Propose(ctx context.Context, a *HSNode) (resp *Empty, err error) {
	...
	for _, n := range c.nodes {
		go callGRPCPropose(ctx, n, a, replyChan)
	}
...
func callGRPCPropose(ctx context.Context, node *Node, arg *HSNode, replyChan chan<- internalPartialCert) {
	reply := new(PartialCert)
	start := time.Now()
	err := grpc.Invoke(
		ctx,
		"/proto.Hotstuff/Propose",
		arg,
		reply,
		node.conn,
	)
	...
	replyChan <- internalPartialCert{node.id, reply, err}
}
