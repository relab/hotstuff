...
/* Exported types and methods for quorum call method Broadcast */
...
func (c *Configuration) Broadcast(ctx context.Context, a *Msg) (resp *QuorumCert, err error) {
	...
	expected := c.n
	replyChan := make(chan internalMsg, expected)
	for _, n := range c.nodes {
		go callGRPCBroadcast(ctx, n, a, replyChan)
	}

	var (
		replyValues = make([]*Msg, 0, expected)
		errs        []GRPCError
		quorum      bool
	)
...
func callGRPCBroadcast(ctx context.Context, node *Node, arg *Msg, replyChan chan<- internalMsg) {
	reply := new(Msg)
	start := time.Now()
	err := grpc.Invoke(
		ctx,
	...
	replyChan <- internalMsg{node.id, reply, err}
}
