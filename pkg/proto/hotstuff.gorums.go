...
type Manager struct {
	...
	proposeID   uint32
	proposeRecv map[uint32]chan *internalPartialCert
...
func NewManager(nodeAddrs []string, opts ...ManagerOption) (*Manager, error) {
	...
	m := &Manager{
		...
		proposeRecv: make(map[uint32]chan *internalPartialCert),
	}
...
func (m *Manager) nextProposeID() uint32 {
	return atomic.AddUint32(&m.proposeID, 1)
}
