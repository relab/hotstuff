...
type CryptoBase interface {
	...
	Sign(message []byte) (signature msg.QuorumSignature, err error)
	// Combine combines multiple signatures into a single signature.
	Combine(signatures ...msg.QuorumSignature) (signature msg.QuorumSignature, err error)
	// Verify verifies the given quorum signature against the message.
	Verify(signature msg.QuorumSignature, message []byte) bool
	// BatchVerify verifies the given quorum signature against the batch of messages.
	BatchVerify(signature msg.QuorumSignature, batch map[hotstuff.ID][]byte) bool
}
