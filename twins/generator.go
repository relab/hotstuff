...
func NewGenerator(logger logging.Logger, numNodes, numTwins, partitions, rounds uint8) *Generator {
	...
	for _, node := range nodes {
		g.leaders = append(g.leaders, node.ReplicaID)
	}

	g.partitionScenarios = genPartitionScenarios(twins, nodes, partitions, 1)

	g.remaining = int64(math.Pow(float64(len(g.partitionScenarios)*len(g.leaders)), float64(g.settings.Rounds)))
...
func (g *Generator) Shuffle(seed int64) {
	...
	r.Shuffle(len(g.partitionScenarios), func(i, j int) {
		g.partitionScenarios[i], g.partitionScenarios[j] = g.partitionScenarios[j], g.partitionScenarios[i]
	})
	for i := range g.offsets {
		g.offsets[i] = r.Intn(len(g.partitionScenarios) * len(g.leaders))
	}
}
...
func (g *Generator) view(index int) View {
	return View{
		Partitions: g.partitionScenarios[index/len(g.leaders)],
		Leader:     g.leaders[index%len(g.leaders)],
	}
}

func (g *Generator) numViews() int {
	return len(g.partitionScenarios) * len(g.leaders)
}
...
func (g *Generator) NextScenario() (s Scenario, err error) {
	...
	p := make(Scenario, g.settings.Rounds)
	...
	for i := int(g.settings.Rounds) - 1; i >= 0; i-- {
		g.indices[i]++
		if g.indices[i] < g.numViews() {
			break
		}
		g.indices[i] = 0
		if i <= 0 {
