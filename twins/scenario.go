...
func ExecuteScenario(scenario Scenario, numNodes, numTwins uint8, consensusName string) (result ScenarioResult, err error) {
	...
	network := newNetwork(scenario, hs.ProposeMsg{}, hs.VoteMsg{}, hs.Hash{})
...
func checkCommits(network *network) (safe bool, commits int) {
	...
	for {
		noCommits := true
		commitCount := make(map[hs.Hash]int)
...
func (lr leaderRotation) GetLeader(view hs.View) hotstuff.ID {
	// we start at view 1
	v := int(view) - 1
	if v >= 0 && v < len(lr) {
		return lr[v].Leader
...
func (cg *commandGenerator) next() hs.Command {
	cg.mut.Lock()
	defer cg.mut.Unlock()
	cmd := hs.Command(strconv.FormatUint(cg.nextCmd, 10))
	cg.nextCmd++
	return cmd
}
...
func (cm commandModule) Get(_ context.Context) (cmd hs.Command, ok bool) {
	return cm.commandGenerator.next(), true
}

// Exec executes the given command.
func (cm commandModule) Exec(block *hs.Block) {
	cm.node.executedBlocks = append(cm.node.executedBlocks, block)
}

func (commandModule) Fork(block *hs.Block) {}
